...
exports.updateprofile = async (req, res) => {
  try {
    ...
    if (profile) {
      const uploadImage = await cloudinary.uploader.upload(profile, { upload_preset: 'moon_dev' });
      data.profile = uploadImage.secure_url;
      data.profile_public_id = uploadImage.public_id;
    }
    const user = await User.findOneAndUpdate({ _id: req.user.id }, data, { returnDocument: 'after' }).select('fullname profile profile_public_id');
